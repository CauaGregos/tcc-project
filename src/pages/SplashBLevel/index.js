...
const SplashBLevel = (props) => {
  ...
  const nextLevel = () => {
    ...
    if((props.route.params?.question-1)%3==0){
      if(props.route.params?.question == 31){axios.put('https://app-tc.herokuapp.com/upgradeProgress',{progress:10,email:email,planet:String(props.route.params?.planet).toLowerCase()}).then((res) => {}).catch((err) => {});}
